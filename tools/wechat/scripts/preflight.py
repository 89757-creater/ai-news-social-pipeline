...
def main():
    ...
    # 修正（2026-09-19）：原先用 Path(cover).is_file()，以 CWD 为基准，
    # 工作目录不是稿件目录时必然误报「缺失」。改为与正文图同一基准：md.parent。
    _cov = Path(cover)
    _cov_abs = _cov if _cov.is_absolute() else (md.parent / _cov)
    cover_ok = bool(cover) and _cov_abs.is_file()
