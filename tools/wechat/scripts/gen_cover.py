...
try:
    import cairosvg
except Exception as _cairo_err:  # 缺 libcairo 动态库时（Windows 常见）给出可执行指引
    raise SystemExit(
        "ERROR: cairosvg 不可用，无法把 SVG 光栅化为 PNG。\n"
        "  原因通常是缺少 libcairo 动态库（Windows 上默认没有）。\n"
        "  解决（二选一）：\n"
        "   1) 直接用纯 Pillow 版封面生成器（无需 cairo）：\n"
        "        python scripts/gen_cover_pillow.py --out cover.png "
        "--eyebrow \"...\" --title \"...\" --subtitle \"...\" --pills \"a,b,c\"\n"
        "   2) 安装 cairo 运行库后重试本脚本。\n"
        f"  原始错误: {_cairo_err}"
    )
